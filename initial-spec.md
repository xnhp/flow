...
Stages are sap workspace paths. Transitions describe either movement between stages or remote sink side effects: a source workspace, optional destination workspace (for move transitions), optionally a condition (sap query expression), and optionally an executable.


# Transition effect

Transitions support two effects:

- **move** (default): current behavior. Read from `from`, optionally transform via action, import to `to`, remove from `from`.
- **sink**: side-effect transition. Read from `from`, run action for remote side effects, do not import to any destination and do not remove from source.
...
For sync-style pipelines keyed by `_sync.id`, `flow` also does conflict preflight on duplicates:

- If multiple local entities share the same `_sync.id`, that id is considered conflicted.
- Pull/source import skips items with conflicted `_sync.id` and emits a warning.
- Sink transitions reject entities with conflicted `_sync.id` and continue with non-conflicted entities.
