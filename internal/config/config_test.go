...
func TestLoadTransitionCriteria(t *testing.T) {
	yaml := `
stages:
  - workspace: ./a
  - workspace: ./b
transitions:
  - from: a
    to: b
    criteria_run: ./scripts/needs-review
    criteria_args:
      dry-run: "true"
`
	dir := t.TempDir()
	...
	if p.Transitions[0].CriteriaRun != "./scripts/needs-review" {
		t.Fatalf("criteria_run = %q", p.Transitions[0].CriteriaRun)
	}
	if p.Transitions[0].CriteriaArgs["dry-run"] != "true" {
		t.Fatalf("criteria_args[dry-run] = %q", p.Transitions[0].CriteriaArgs["dry-run"])
	}
}

func TestLoadTransitionCriteriaArgsRequireCriteriaRun(t *testing.T) {
	yaml := `
stages:
  - workspace: ./a
  - workspace: ./b
...
`
	...
	_, _, err := Load(path)
	if err == nil {
		t.Fatal("expected error for missing criteria_run")
	}
}
